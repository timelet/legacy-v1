    ...
    if (database) {
      database.entries.find().$.subscribe((docs) => {
        setRows(docs.map((doc, i) => ({
          ...
          endedAt: doc.endedAt ? `${intl.formatDate(doc.endedAt)} ${intl.formatTime(doc.endedAt)}` : intl.formatMessage({id: "label.undefined", defaultMessage: "Undefined", description: "An undefined value"})
        })));
      });
    }
  }, [database]);
